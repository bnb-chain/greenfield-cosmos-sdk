...
func TestGasKVStoreBasic(t *testing.T) {
	...
	st.Delete(keyFmt(1))
	require.Empty(t, st.Get(keyFmt(1)), "Expected `key1` to be empty")
	require.Equal(t, meter.RwConsumed(), types.Gas(6858))
}
...
func TestGasKVStoreIterator(t *testing.T) {
	...
	iterator.Next()
	require.Equal(t, types.Gas(14565), meter.RwConsumed())
	kc := iterator.Key()
	require.Equal(t, kc, keyFmt(3))
	vc := iterator.Value()
	require.Equal(t, vc, valFmt(0))
	iterator.Next()
	require.Equal(t, types.Gas(14667), meter.RwConsumed())
	require.False(t, iterator.Valid())
	require.Panics(t, iterator.Next)
	require.Equal(t, types.Gas(14697), meter.RwConsumed())
	require.NoError(t, iterator.Error())
	...
	require.Equal(t, types.Gas(15135), meter.RwConsumed())
}

// no limit on r/w consumed
