...
func TestImportExportQueues(t *testing.T) {
	...
	ctx := s1.app.BaseApp.NewContext(false, cmtproto.Header{ChainID: testutil.DefaultChainId})
	addrs := simtestutil.AddTestAddrs(s1.BankKeeper, s1.StakingKeeper, ctx, 1, valTokens)

	header := cmtproto.Header{ChainID: testutil.DefaultChainId, Height: s1.app.LastBlockHeight() + 1}
	...
	s2.app.Commit()
	s2.app.BeginBlock(abci.RequestBeginBlock{Header: cmtproto.Header{ChainID: testutil.DefaultChainId, Height: s2.app.LastBlockHeight() + 1}})

	header = cmtproto.Header{ChainID: testutil.DefaultChainId, Height: s2.app.LastBlockHeight() + 1}
