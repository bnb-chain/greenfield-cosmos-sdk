...
Example:
$ %s query distribution validator-outstanding-rewards 0x91D7d..
`,
				version.AppName,
			),
		),
...
Example:
$ %s query distribution commission 0x91D7d..
`,
				version.AppName,
			),
		),
...
Example:
$ %s query distribution slashes 0x91D7d.. 0 100
`,
				version.AppName,
			),
		),
...
Example:
$ %s query distribution rewards 0x9fB29..
$ %s query distribution rewards 0x9fB29.. 0x91D7d..
`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			ctx := cmd.Context()
			if len(args) == 2 {
				validatorAddr, err := sdk.AccAddressFromHexUnsafe(args[1])
				if err != nil {
					return err
				}
