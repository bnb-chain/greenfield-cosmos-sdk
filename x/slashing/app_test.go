...
func TestSlashingMsgs(t *testing.T) {
	...
	_, _, err = simapp.SignCheckDeliver(t, txGen, app.BaseApp, header, []sdk.Msg{createValidatorMsg}, "", []uint64{0}, []uint64{0}, true, true, []cryptotypes.PrivKey{priv1})
	require.NoError(t, err)
	...
	header = tmproto.Header{Height: app.LastBlockHeight() + 1}
	_, res, err := simapp.SignCheckDeliver(t, txGen, app.BaseApp, header, []sdk.Msg{unjailMsg}, "", []uint64{0}, []uint64{1}, false, false, []cryptotypes.PrivKey{priv1})
	require.Error(t, err)
	require.Nil(t, res)
