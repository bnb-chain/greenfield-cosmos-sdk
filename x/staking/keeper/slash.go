...
func (k Keeper) SlashRedelegation(ctx sdk.Context, srcValidator types.Validator, redelegation types.Redelegation,
	...
	for _, entry := range redelegation.Entries {
		...
		unbondingDelegation, found := k.GetUnbondingDelegation(ctx, sdk.MustAccAddressFromHex(redelegation.DelegatorAddress), sdk.MustAccAddressFromHex(validatorDstAddress.String()))
		if found {
			for i, entry := range unbondingDelegation.Entries {
		...
		delegatorAddress := sdk.MustAccAddressFromHex(redelegation.DelegatorAddress)

		delegation, found := k.GetDelegation(ctx, delegatorAddress, sdk.AccAddress(valDstAddr))
		if !found {
		...
		tokensToBurn, err := k.Unbond(ctx, delegatorAddress, sdk.AccAddress(valDstAddr), sharesToUnbond)
		if err != nil {
			panic(fmt.Errorf("error unbonding delegator: %v", err))
		}

		dstValidator, found := k.GetValidator(ctx, sdk.AccAddress(valDstAddr))
		if !found {
