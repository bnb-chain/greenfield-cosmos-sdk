...
func initValidators(t testing.TB, power int64, numAddrs int, powers []int64) (*simapp.SimApp, sdk.Context, []sdk.AccAddress, []sdk.AccAddress, []types.Validator) {
	...
	for i, power := range powers {
		vs[i] = teststaking.NewValidator(t, sdk.AccAddress(addrs[i]), pks[i])
...
func TestGetValidatorSortingUnmixed(t *testing.T) {
	...
	for i, amt := range amts {
		validators[i] = teststaking.NewValidator(t, addrs[i], PKs[i])
...
func TestGetValidatorSortingMixed(t *testing.T) {
	...
	val0, found := app.StakingKeeper.GetValidator(ctx, addrs[0])
	require.True(t, found)
	val1, found := app.StakingKeeper.GetValidator(ctx, addrs[1])
	require.True(t, found)
	val2, found := app.StakingKeeper.GetValidator(ctx, addrs[2])
	require.True(t, found)
	val3, found := app.StakingKeeper.GetValidator(ctx, addrs[3])
	require.True(t, found)
	val4, found := app.StakingKeeper.GetValidator(ctx, addrs[4])
	require.True(t, found)
...
func TestValidatorBondHeight(t *testing.T) {
	...
	// initialize some validators into the state
	var validators [3]types.Validator
	validators[0] = teststaking.NewValidator(t, PKs[0].Address().Bytes(), PKs[0])
	validators[1] = teststaking.NewValidator(t, addrs[1], PKs[1])
	validators[2] = teststaking.NewValidator(t, addrs[2], PKs[2])
